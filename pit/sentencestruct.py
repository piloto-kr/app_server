...
#for sentence struct
def makeResultString(corpus):
    retstr = ''
    clist = checkSSList(corpus)
    for cond in clist:
        if cond:
            retstr.append('1')
        else
...
def findPattern(p, sp, index1=0, index2=0):
    if index2 >= len(sp):
        return True
    elif index1 >= len(p):
        return False
    ...
    if p[index1] == sp[index2]:
        return findPattern(p, sp, index1+1, index2+1)
    else:
        return findPattern(p, sp, index1+1)

def getPosPattern(pos, opt=0):
    pospat = []
    for morphs in pos:
        if opt:
...
def checkSSList(corpus):
    kkma = Kkma()
    pos = kkma.pos(corpus)
    posn = kkma.nouns(corpus)
    words = getWords(pos)
    pospat = getPosPattern(pos)
    pospatu = getPosPattern(pos, 1)
    ...
    #c certain thing
    cnn = findPattern(pospatu, ['N','N'])
    cmn = findPattern(pospatu, ['M','N'])
    cven = findPattern(pospat, ['VA','ETD','NNG'])
    cven = cven or findPattern(pospat, ['VA', 'ETD', 'NN'])
    cven = cven or findPattern(pospat, ['VA', 'ETD', 'NNP'])
    ...
    cgoit = findPattern(pos, [('고', 'ECE'), ('있', 'VV')])
    cjoongi = findPattern(pos, [('중', 'NNB'), ('이', 'VCP')])
    ...
    for m in pos_r:
        pos_an.remove(m)
    pos_an = getPosPattern(pos_an)
    can = findPattern(pos_an, ['MAG', 'VV'])
    can = can or findPattern(pos_an, ['MAG', 'VA'])
    ...
    ctwp = len(words) >= 3
    cdesc = findPattern(getPosPattern(words, 1), ['M', 'V', 'N'])
    ...
    ckkmaexc = findPattern(pos, [('나', 'VV'), ('는', 'ETD')])
    ...
    cmpp = findPattern(pos, [('그', 'VV'), ('어야', 'ECD')])
    cmpp = cmpp or findPattern(pos, [('의', 'JKG'), ('것', 'NNB')])
    ...
    cgruna = findPattern(pos, [('그러', 'VV'), ('나', 'ECE')])
    cddaemoon = findPattern(pos, [('때문', 'NNB'), ('에', 'JKM')])
    checkQ = candor or cgruna or cddaemoon
        
    #r verbal noun (gerund)
    checkhagi = findPattern(pos, [('하', 'XSV'), ('기', 'ETN')])
    pos_vr = [m for m in pos if m[1] == 'NNB' and m[0] != '것']
    pos_vn = pos.copy()
    for m in pos_vr:
        pos_vn.remove(m)
    pos_vn = getPosPattern(pos_vn)
    checkgut = findPattern(pos_vn, ['VV','ETD','NNB'])
    checkhgut = findPattern(pos_vn, ['XSV','ETD','NNB'])
